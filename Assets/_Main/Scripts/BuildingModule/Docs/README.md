...
- `BuildingUpgradeController` находит постройку под прицелом в режиме строительства, показывает состояние улучшения и запускает его через Build input.
...
В режиме строительства постройка под центром камеры подсвечивается: циановым цветом при доступной покупке и красным при нехватке ресурсов. Клавиша `F` (`Build/Upgrade`) улучшает только выбранную постройку; сервис повторно проверяет цену и доступность. На максимальном уровне подсветка убирается.
