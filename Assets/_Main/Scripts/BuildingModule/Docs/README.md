...
Панель открывается после `ExternalUIManager.UIOpened` для построек с `IExternalUI` (верстак или сундук) и не заменяет существующее окно. Постройка без внешнего UI не объявляется интерактивной только ради улучшения: это не создаёт панель без активного UI-контекста.
