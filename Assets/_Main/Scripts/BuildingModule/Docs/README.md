...
- `BuildingUpgradeController` находит постройку под прицелом в режиме строительства и запускает улучшение через Build input. `BuildingUpgradeHoldModel` считает прогресс удержания, а `BuildingUpgradePresenter` обновляет компактный HUD.
...
В режиме строительства под центром камеры показывается полупрозрачное превью следующего уровня: зеленое при доступной покупке и красное при нехватке ресурсов. HUD показывает цену следующего уровня как текущее/необходимое количество материалов. Удержание `F` (`Build/Upgrade`) в течение 1,5 секунд улучшает только выбранную постройку; прогресс сбрасывается при отпускании клавиши, смене цели, выходе из режима или паузе. Сервис повторно проверяет цену и доступность. На максимальном уровне подсветка и HUD убираются.
