...
{
    /// <summary>Marks a spawnable event site prefab with its guard, occupant and loot points and its footprint.</summary>
    /// <remarks>
    /// Authored once on each site prefab (a crash site, an outpost, a trader camp, and so on). An empty
    /// <see cref="GuardPoints"/>, <see cref="OccupantPoints"/> or <see cref="LootPoints"/> list tells the
    /// spawning action to scatter that group around the site instead of using fixed points.
    ...
    public class EventSiteView : MonoBehaviour
    {
        ...
        [SerializeField] private List<Transform> occupantPoints = new();
        ...
        /// <summary>Points where the site's own non-hostile characters stand; empty means scatter around the site.</summary>
        public List<Transform> OccupantPoints => occupantPoints;
